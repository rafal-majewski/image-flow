...
export class FuzzyGameOfLifeMapper extends Mapper {
	private readonly mixFactor: number;
	public constructor(mixFactor: number) {
		super("fuzzy-game-of-life", "Game of Life (fuzzy)");
		...
		for (;;) {
			...
			for (
				...
				const color: RgbColor = {
					red: (1 - Math.min(1 * Math.abs(neighborCount.red - 2.5), 1)) * 255,
					green:
						(1 - Math.min(1 * Math.abs(neighborCount.green - 2.5), 1)) * 255,
					blue: (1 - Math.min(1 * Math.abs(neighborCount.blue - 2.5), 1)) * 255,
				};
