...
>
	...
	import {AnimatedOperatingDoneOperatableNodeState} from "../implementations/animated-operating-done/AnimatedOperatingDoneOperatableNodeState.ts";
	import {AnimatedOperatingInProgressOperatableNodeState} from "../implementations/animated-operating-in-progress/AnimatedOperatingInProgressOperatableNodeState.ts";
	...
	import {InstantOperatingDoneOperatableNodeState} from "../implementations/instant-operating-done/InstantOperatingDoneOperatableNodeState.ts";
	...
	import {ManualOperatingDoneOperatableNodeState} from "../implementations/manual-operating-done/ManualOperatingDoneOperatableNodeState.ts";
	import {ManualOperatingInProgressOperatableNodeState} from "../implementations/manual-operating-in-progress/ManualOperatingInProgressOperatableNodeState.ts";
	const {
		state,
...
<section>
	<select onchange={handleSelectChange}>
		...
		{#each operators as operator (operator.id)}
			<option
				value={operator.id}
				selected={(state
					instanceof AnimatedOperatingInProgressOperatableNodeState
					|| state instanceof AnimatedOperatingDoneOperatableNodeState
					|| state instanceof AnimatedInvalidOperatableNodeState
					|| state instanceof InstantOperatingDoneOperatableNodeState
					|| state instanceof InstantInvalidOperatableNodeState
					|| state instanceof ManualOperatingInProgressOperatableNodeState
					|| state instanceof ManualOperatingDoneOperatableNodeState
					|| state instanceof ManualInvalidOperatableNodeState)
					&& state.operator.id === operator.id}>{operator.name}</option
			>
		{/each}
	</select>
	{#if state instanceof AnimatedOperatingInProgressOperatableNodeState || state instanceof AnimatedOperatingDoneOperatableNodeState || state instanceof AnimatedInvalidOperatableNodeState || state instanceof AnimatedInvalidOperatableNodeState || state instanceof InstantOperatingDoneOperatableNodeState || state instanceof InstantInvalidOperatableNodeState || state instanceof InstantInvalidOperatableNodeState || state instanceof ManualOperatingInProgressOperatableNodeState || state instanceof ManualOperatingDoneOperatableNodeState || state instanceof ManualInvalidOperatableNodeState || state instanceof ManualInvalidOperatableNodeState}
		<state.operator.displayer {onSetOperatorRequest} {nodeId} />
	{/if}
	{#if state instanceof AnimatedOperatingInProgressOperatableNodeState || state instanceof AnimatedOperatingDoneOperatableNodeState || state instanceof InstantOperatingDoneOperatableNodeState || state instanceof ManualOperatingInProgressOperatableNodeState || state instanceof ManualOperatingDoneOperatableNodeState}
	...
	<fieldset>
		...
		<div>
			<label>
				<input
					...
					name="{nodeId}-mode"
					value="manual"
					checked={state instanceof ManualOperatingInProgressOperatableNodeState
						|| state instanceof ManualOperatingDoneOperatableNodeState
						|| state instanceof ManualInvalidAndNoOperatorOperatableNodeState
						|| state instanceof ManualInvalidOperatableNodeState
						|| state instanceof ManualNoOperatorOperatableNodeState}
			...
			<label>
				<input
					...
					name="{nodeId}-mode"
					value="instant"
					checked={state instanceof InstantOperatingDoneOperatableNodeState
						|| state instanceof InstantInvalidAndNoOperatorOperatableNodeState
						|| state instanceof InstantInvalidOperatableNodeState
						|| state instanceof InstantNoOperatorOperatableNodeState}
		...
		{#if state instanceof ManualOperatingInProgressOperatableNodeState || state instanceof ManualOperatingDoneOperatableNodeState || state instanceof ManualInvalidAndNoOperatorOperatableNodeState || state instanceof ManualInvalidOperatableNodeState || state instanceof ManualNoOperatorOperatableNodeState || state instanceof AnimatedOperatingInProgressOperatableNodeState || state instanceof AnimatedOperatingDoneOperatableNodeState || state instanceof AnimatedInvalidAndNoOperatorOperatableNodeState || state instanceof AnimatedInvalidOperatableNodeState || state instanceof AnimatedNoOperatorOperatableNodeState}
			<div>
				{#if state instanceof ManualOperatingInProgressOperatableNodeState || state instanceof ManualOperatingDoneOperatableNodeState || state instanceof ManualInvalidAndNoOperatorOperatableNodeState || state instanceof ManualInvalidOperatableNodeState || state instanceof ManualNoOperatorOperatableNodeState}
					<label>
						Step count:
						<input
							type="number"
				...
				{/if}
				{#if state instanceof AnimatedOperatingInProgressOperatableNodeState || state instanceof AnimatedOperatingDoneOperatableNodeState || state instanceof AnimatedInvalidAndNoOperatorOperatableNodeState || state instanceof AnimatedInvalidOperatableNodeState || state instanceof AnimatedNoOperatorOperatableNodeState}
					<label>
						Interval (seconds):
						<input
							type="number"
